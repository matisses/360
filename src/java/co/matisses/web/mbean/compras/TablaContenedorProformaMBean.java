 ...
 */
@ViewScoped
@Named(value = "tablaContenedorProformaMBean")
public class TablaContenedorProformaMBean implements Serializable {

    @Inject
    private ProformaSessionMBean proformaSession;
    @Inject
    private BaruApplicationMBean aplicacionBean;
    private static final Logger log = Logger.getLogger(TablaContenedorProformaMBean.class.getSimpleName());
    ...
    public TablaContenedorProformaMBean() {
    ...
    protected void initialize() {
        ...
        if (idProforma != null && idProforma != 0) {
            ...
            mostrarMensaje("Error", "No se encontró una proforma seleccionada.", true, false, false);
            log.log(Level.SEVERE, "No se encontro una proforma seleccionada");
            return;
        }
        idContenedorProforma = Integer.parseInt(FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("idContenedorProforma"));
        if (idContenedorProforma == null || idContenedorProforma == 0) {
            mostrarMensaje("Error", "No se encontró un contenedor seleccionado de la proforma [" + proforma.getIdProforma() + "].", true, false, false);
            log.log(Level.SEVERE, "No se encontro un contenedor seleccionado de la proforma [{0}]", proforma.getIdProforma());
            return;
        }
    ...
    private void obtenerDatosProforma() {
        System.out.println("Inicia obtencion de datos de la tabla de datos contenedor");
        obtenerConfiguracionMostrar();
        obtenerLineasIgnoradas();

        List<Object[]> lineas = contenedorLineaFacade.obtenerCantidadesLineaContenedor(idContenedorProforma);

        if (lineas != null && !lineas.isEmpty()) {
            for (Object[] i : lineas) {
                DatosProformaDTO d = new DatosProformaDTO((Integer) i[0], validarLineaIncluida((Integer) i[0]), new ArrayList<DetalleProformaDTO>());
        ...
        System.out.println("Finaliza obtencion de datos de la tabla de datos contenedor");
    }

    private void obtenerConfiguracionMostrar() {
        ...
        if (confs != null && !confs.isEmpty()) {
            ...
            /*Se agrega columna de cantidad contenedor*/
            encabezado.add(new ColumnaDatosProformaDTO(encabezado.size() + 1, "100px", "Cantidad", null, new ArrayList<ColumnaProformaDTO>()));

            encabezado.get(encabezado.size() - 1).getColumnas().add(new ColumnaProformaDTO(encabezado.size(), null, null, 2, null, null, "Contenedor", null, "100px", false, false,
    ...
    public boolean validarLineaIncluida(int linea) {
    ...
    public String verFoto(String img) {
    ...
    public void mandarDatosLineaSeleccionada() {
        ...
        if (linea != null && linea > -1) {
            proformaSession.setLinea(linea);
