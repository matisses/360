 ...
 * @author ygil
 */
public class IncomingPaymentClient {

    private WebTarget webTarget;
    private Client client;

    public IncomingPaymentClient(String BASE_URI) {
        client = ClientBuilder.newClient();
        webTarget = client.target(BASE_URI).path("incomingpayment");
    }

    public IncomingPaymentClient(String BASE_URI, String path) {
        client = ClientBuilder.newClient();
        webTarget = client.target(BASE_URI).path(path);
    }

    public GenericRESTResponseDTO crearPago(String usuario, PaymentDTO dto) {
